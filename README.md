...
## PASSO 3: Crie o documento .env:

O arquivo `.env` é essencial para configurar as variáveis de ambiente utilizadas no projeto. Essas variáveis ajudam a manter informações sensíveis, como chaves de API e strings de conexão com o banco de dados, fora do código-fonte principal, oferecendo maior segurança e flexibilidade.

A `STRING_CONEXAO` e uma string dada pelo mogoDB para conexão, foi ultilizado o Atlas [mogoDB](https://www.mongodb.com/products/platform/atlas-database)

A `GEMINI_API_KEY` e a KEY da api da Gemini.
...
Conectado ao cluster do banco de dados...
